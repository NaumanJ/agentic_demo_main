...
def insert_documents_to_milvus(collection, documents, embedding_model):
    ...
    entities = [
        ...
        {"name": "content", "values": [doc['content'] for doc in documents]}
    ]
...
class Neo4jKnowledgeGraph:
    ...
    def _create_document_node(tx, title, content):
        tx.run("CREATE (d:Document {title: $title, content: $content})", title=title, content=content)
...
def generate_llm_response_with_rag(model, tokenizer, question, schemas, pg_engine, neo4j_docs, max_length=500):
    """
    Generates a response to the user's question using a RAG (Retrieval-Augmented Generation) pipeline.
    Retrieves relevant documents from various data sources are provided by the user for RAG.
    Prepare a response in a humourous tone with no illigible context or illegal keywords in the response.
    """
    ...
    #     schema_info += f"Table {table_name}: columns are {', '.join(columns)}.\n"

    # Prepare prompt
    retrieved_context = "\n".join(postgres_docs + neo4j_docs_retrieved)
    prompt = (
        f"Question: {question}\n\n"
        f"Schema Information:\n{schema_info}\n\n"
        f"Retrieved Context:\n{retrieved_context}\n\n"
        f"Generate a detailed answer using the provided context."
    )
    ...
    # Generate response without gradients
...
def main():
    ...
    st.title("Sports Chatbot with PostgreSQL, Neo4j, and Milvus")
    logging.info("Initializing PostgreSQL engine and retrieving schema.")
    
    try:
        # Initialize PostgreSQL engine and retrieve schema
    ...
    try:
        ...
        logging.info("LLM model and tokenizer loaded successfully.")
    except Exception as e:
        logging.error(f"Error loading LLM model and tokenizer: {e}")
    ...
    if st.button("Submit"):
        if question:
            try:
                ...
                logging.info("Retrieving relevant documents from Milvus based on vector similarity.")
                milvus_results = search_documents_milvus(collection, question, embedding_model)
                logging.info(f"Documents retrieved from Milvus: {len(milvus_results)} document(s) found.")
                
                # Step 2: Extract keywords from the question for PostgreSQL and Neo4j
                logging.info("Extracting keywords from question and transcript.")
                combined_text = transcript_sample + '\n' + question
                keywords = extract_keywords(combined_text)
                logging.info(f"Keywords extracted: {keywords}")

                # Display word cloud for keywords
                display_word_cloud(keywords)

                # Step 3: Retrieve documents from Neo4j
                logging.info("Searching for documents in Neo4j based on extracted keywords.")
                neo4j_docs = graph.search_documents(keywords)
                logging.info(f"Documents retrieved from Neo4j: {len(neo4j_docs)} document(s) found.")

                # Step 4: Generate response using the RAG pipeline (PostgreSQL, Neo4j, and Milvus)
                ...
                response = generate_llm_response_with_rag(
                    ...
                    schemas,
                    pg_engine,
                    milvus_results + neo4j_docs
                )
                logging.info("Response generated successfully.")

                # Step 5: Display the LLM-generated response
                st.subheader("LLM Response")
                st.text_area("Response", response, height=300)

                # Step 6: Display retrieved documents (if any)
                st.subheader("Retrieved Documents")
                for doc in milvus_results + neo4j_docs:
                    st.write(doc['content'] if 'content' in doc else doc)
