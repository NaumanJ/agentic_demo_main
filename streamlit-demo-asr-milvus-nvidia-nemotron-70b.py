...
def generate_llm_response_with_rag(model, tokenizer, question, combined_docs, max_length=100):
    ...
    # Combine all the documents into a single context
    retrieved_context = "\n".join(combined_docs)
    question = "Answer really concise in 2 sentences." + question
...
# def main():
#     """
#     Main function to run the Sports Chatbot with video and LLM search functionality.
#     """
...
#         """
#         <style>
#         .stApp { ... } /* Your existing CSS */
#         </style>
#         """,
#         unsafe_allow_html=True,
#     )

#     # Sidebar for video selection and branding
...
#         st.sidebar.write(f"Current Video: *{selected_video}*")
#         selected_video = 'soccerMatch.mp4'
#         if selected_video:
#             play_video_loop(selected_video, video_dir)
#     except Exception as e:
...
#             # Handle video query
#             video_handled = handle_video_query(question, selected_video, video_dir, annotation_loader)
#             # if not video_handled:
#             # Proceed with LLM search if the question is not video-specific
#             keywords = extract_keywords(question)
#             logging.info(f"LLM search keywords: {keywords}")
...
#             # Combine documents
#             combined_docs = (milvus_docs or []) + (postgres_docs or []) + (neo4j_docs or [])
...
def main():
    ...
    # Custom CSS for fuchsia background and chat log styling
    st.markdown(
        """
        <style>
        /* Main window background and chat log styling */
        .stApp { ... } /* Your existing CSS */
        </style>
        """,
        unsafe_allow_html=True,
